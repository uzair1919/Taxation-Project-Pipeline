...
def GRD_toRGB_S2(path_S2):
    ...
    x_img[:, :, 3] = cv2.resize(s2_img[:, :, 3], (IMG_SIZE[0], IMG_SIZE[1]), interpolation=cv2.INTER_AREA)  # Changed from 6
    x_img[:, :, 4] = cv2.resize(s2_img[:, :, 4], (IMG_SIZE[0], IMG_SIZE[1]), interpolation=cv2.INTER_AREA)  # Changed from 9
